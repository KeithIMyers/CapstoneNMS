...
@section('head_title', 'Create account · '.getcong('site_name'))

@section('content')
<div class="container-narrow" style="margin-block: 3rem 4rem;">
    <h1 style="text-align:center; margin-bottom: 1.5rem;">Create your account</h1>

    @if ($errors->any())
        <div class="flash flash--err">{{ $errors->first() }}</div>
    @endif

    <form method="post" action="{{ route('user_signup') }}" class="card" style="padding: 1.5rem;">
        @csrf
        ...
        @if (getcong('recaptcha_on_signup') && getcong('recaptcha_site_key'))
    ...
    <p class="text-muted text-center" style="margin-top: 1rem;">
        Already have an account? <a href="{{ route('user_login') }}">Sign in</a>
    </p>
</div>
@endsection
